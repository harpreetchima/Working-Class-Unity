---
url: "https://nodejs.org/dist/latest/docs/api/sqlite.json"
title: undefined
---
      ...
      "classes": [
        {
          ...
          "methods": [
            ...
            {
              "textRaw": "`database.enableDefensive(active)`",
              "type": "method",
              "name": "enableDefensive",
              "meta": {
                "added": [
                  "v25.1.0"
                ],
            ...
            {
              "textRaw": "`database.setAuthorizer(callback)`",
              "type": "method",
              "name": "setAuthorizer",
              "meta": {
                "added": [
              ...
              "signatures": [
                {
                  "params": [
                    {
                      "textRaw": "`callback` {Function|null} The authorizer function to set, or `null` to clear the current authorizer.",
                      "name": "callback",
                      "type": "Function|null",
                      "desc": "The authorizer function to set, or `null` to clear the current authorizer."
                    }
                  ]
                }
              ],
              "desc": "<p>Sets an authorizer callback that SQLite will invoke whenever it attempts to\naccess data or modify the database schema through prepared statements.\nThis can be used to implement security policies, audit access, or restrict certain operations.\nThis method is a wrapper around <a href=\"https://sqlite.org/c3ref/set_authorizer.html\"><code>sqlite3_set_authorizer()</code></a>.</p>\n<p>When invoked, the callback receives five arguments:</p>\n<ul>\n<li><code>actionCode</code> <a href=\"https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#number_type\" class=\"type\">&lt;number&gt;</a> The type of operation being performed (e.g.,\n<code>SQLITE_INSERT</code>, <code>SQLITE_UPDATE</code>, <code>SQLITE_SELECT</code>).</li>\n<li><code>arg1</code> <a href=\"https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#string_type\" class=\"type\">&lt;string&gt;</a> | <a href=\"https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#null_type\" class=\"type\">&lt;null&gt;</a> The first argument (context-dependent, often a table name).</li>\n<li><code>arg2</code> <a href=\"https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#string_type\" class=\"type\">&lt;string&gt;</a> | <a href=\"https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#null_type\" class=\"type\">&lt;null&gt;</a> The second argument (context-dependent, often a column name).</li>\n<li><code>dbName</code> <a href=\"https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#string_type\" class=\"type\">&lt;string&gt;</a> | <a href=\"https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#null_type\" class=\"type\">&lt;null&gt;</a> The name of the database.</li>\n<li><code>triggerOrView</code> <a href=\"https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#string_type\" class=\"type\">&lt;string&gt;</a> | <a href=\"https://developer.mozilla.org/en-US/docs/Web/JavaScript/Data_structures#null_type\" class=\"type\">&lt;null&gt;</a> The name of the trigger or view causing the access.</li>\n</ul>\n<p>The callback must return one of the following constants:</p>\n<ul>\n<li><code>SQLITE_OK</code> - Allow the operation.</li>\n<li><code>SQLITE_DENY</code> - Deny the operation (causes an error).</li>\n<li><code>SQLITE_IGNORE</code> - Ignore the operation (silently skip).</li>\n</ul>\n<pre><code class=\"language-cjs\">const { DatabaseSync, constants } = require('node:sqlite');\nconst db = new DatabaseSync(':memory:');\n\n// Set up an authorizer that denies all table creation\ndb.setAuthorizer((actionCode) => {\n  if (actionCode === constants.SQLITE_CREATE_TABLE) {\n    return constants.SQLITE_DENY;\n  }\n  return constants.SQLITE_OK;\n});\n\n// This will work\ndb.prepare('SELECT 1').get();\n\n// This will throw an error due to authorization denial\ntry {\n  db.exec('CREATE TABLE blocked (id INTEGER)');\n} catch (err) {\n  console.log('Operation blocked:', err.message);\n}\n</code></pre>\n<pre><code class=\"language-mjs\">import { DatabaseSync, constants } from 'node:sqlite';\nconst db = new DatabaseSync(':memory:');\n\n// Set up an authorizer that denies all table creation\ndb.setAuthorizer((actionCode) => {\n  if (actionCode === constants.SQLITE_CREATE_TABLE) {\n    return constants.SQLITE_DENY;\n  }\n  return constants.SQLITE_OK;\n});\n\n// This will work\ndb.prepare('SELECT 1').get();\n\n// This will throw an error due to authorization denial\ntry {\n  db.exec('CREATE TABLE blocked (id INTEGER)');\n} catch (err) {\n  console.log('Operation blocked:', err.message);\n}\n</code></pre>"
            },
            {
            ...
            {
              "textRaw": "`database.createTagStore([maxSize])`",
              "type": "method",
              "name": "createTagStore",
              "meta": {
                "added": [
              ...
              "signatures": [
                {
                  "return": {
                    "textRaw": "Returns: {SQLTagStore} A new SQL tag store for caching prepared statements.",
                    "name": "return",
                    "type": "SQLTagStore",
                    "desc": "A new SQL tag store for caching prepared statements."
                  },
                  "params": [
                    {
                      "textRaw": "`maxSize` {integer} The maximum number of prepared statements to cache. **Default:** `1000`.",
                      "name": "maxSize",
                      "type": "integer",
                      "default": "`1000`",
                      "desc": "The maximum number of prepared statements to cache."
                    }
                  ]
                }
              ],
              "desc": "<p>Creates a new <code>SQLTagStore</code>, which is an LRU (Least Recently Used) cache for\nstoring prepared statements. This allows for the efficient reuse of prepared\nstatements by tagging them with a unique identifier.</p>\n<p>When a tagged SQL literal is executed, the <code>SQLTagStore</code> checks if a prepared\nstatement for that specific SQL string already exists in the cache. If it does,\nthe cached statement is used. If not, a new prepared statement is created,\nexecuted, and then stored in the cache for future use. This mechanism helps to\navoid the overhead of repeatedly parsing and preparing the same SQL statements.</p>\n<pre><code class=\"language-mjs\">import { DatabaseSync } from 'node:sqlite';\n\nconst db = new DatabaseSync(':memory:');\nconst sql = db.createTagStore();\n\ndb.exec('CREATE TABLE users (id INT, name TEXT)');\n\n// Using the 'run' method to insert data.\n// The tagged literal is used to identify the prepared statement.\nsql.run`INSERT INTO users VALUES (1, 'Alice')`;\nsql.run`INSERT INTO users VALUES (2, 'Bob')`;\n\n// Using the 'get' method to retrieve a single row.\nconst id = 1;\nconst user = sql.get`SELECT * FROM users WHERE id = ${id}`;\nconsole.log(user); // { id: 1, name: 'Alice' }\n\n// Using the 'all' method to retrieve all rows.\nconst allUsers = sql.all`SELECT * FROM users ORDER BY id`;\nconsole.log(allUsers);\n// [\n//   { id: 1, name: 'Alice' },\n//   { id: 2, name: 'Bob' }\n// ]\n</code></pre>\n<pre><code class=\"language-cjs\">const { DatabaseSync } = require('node:sqlite');\n\nconst db = new DatabaseSync(':memory:');\nconst sql = db.createTagStore();\n\ndb.exec('CREATE TABLE users (id INT, name TEXT)');\n\n// Using the 'run' method to insert data.\n// The tagged literal is used to identify the prepared statement.\nsql.run`INSERT INTO users VALUES (1, 'Alice')`;\nsql.run`INSERT INTO users VALUES (2, 'Bob')`;\n\n// Using the 'get' method to retrieve a single row.\nconst id = 1;\nconst user = sql.get`SELECT * FROM users WHERE id = ${id}`;\nconsole.log(user); // { id: 1, name: 'Alice' }\n\n// Using the 'all' method to retrieve all rows.\nconst allUsers = sql.all`SELECT * FROM users ORDER BY id`;\nconsole.log(allUsers);\n// [\n//   { id: 1, name: 'Alice' },\n//   { id: 2, name: 'Bob' }\n// ]\n</code></pre>"
            },
            {
          ...
          "properties": [
            {
              "textRaw": "`isOpen` Type: {boolean} Whether the database is currently open or not.",
              "type": "boolean",
              "name": "Type",
              "meta": {
                "added": [
            ...
            {
              "textRaw": "`isTransaction` Type: {boolean} Whether the database is currently within a transaction. This method is a wrapper around [`sqlite3_get_autocommit()`][].",
              "type": "boolean",
              "name": "Type",
              "meta": {
                "added": [
              ...
              "params": [
                ...
                },
                {
                  ...
                  "options": [
                    ...
                    {
                      "textRaw": "`readBigInts` {boolean} If `true`, integer fields are read as JavaScript `BigInt` values. If `false`, integer fields are read as JavaScript numbers. **Default:** `false`.",
                      "name": "readBigInts",
                      "type": "boolean",
                      "default": "`false`",
                      "desc": "If `true`, integer fields are read as JavaScript `BigInt` values. If `false`, integer fields are read as JavaScript numbers."
                    },
                    {
                      "textRaw": "`returnArrays` {boolean} If `true`, query results are returned as arrays instead of objects. **Default:** `false`.",
                      "name": "returnArrays",
                      "type": "boolean",
                      "default": "`false`",
                      "desc": "If `true`, query results are returned as arrays instead of objects."
                    },
                    {
                      "textRaw": "`allowBareNamedParameters` {boolean} If `true`, allows binding named parameters without the prefix character (e.g., `foo` instead of `:foo`). **Default:** `true`.",
                      "name": "allowBareNamedParameters",
                      "type": "boolean",
                      "default": "`true`",
                      "desc": "If `true`, allows binding named parameters without the prefix character (e.g., `foo` instead of `:foo`)."
                    },
                    {
                      "textRaw": "`allowUnknownNamedParameters` {boolean} If `true`, unknown named parameters are ignored when binding. If `false`, an exception is thrown for unknown named parameters. **Default:** `false`.",
                      "name": "allowUnknownNamedParameters",
                      "type": "boolean",
                      "default": "`false`",
                      "desc": "If `true`, unknown named parameters are ignored when binding. If `false`, an exception is thrown for unknown named parameters."
                    },
                    {
                      "textRaw": "`defensive` {boolean} If `true`, enables the defensive flag. When the defensive flag is enabled, language features that allow ordinary SQL to deliberately corrupt the database file are disabled. The defensive flag can also be set using `enableDefensive()`. **Default:** `false`.",
                      "name": "defensive",
                      "type": "boolean",
                      "default": "`false`",
                      "desc": "If `true`, enables the defensive flag. When the defensive flag is enabled, language features that allow ordinary SQL to deliberately corrupt the database file are disabled. The defensive flag can also be set using `enableDefensive()`."
                    }
                  ]
                }
              ],
        ...
        },
        {
          ...
          "methods": [
            ...
            {
              "textRaw": "`statement.setReturnArrays(enabled)`",
              "type": "method",
              "name": "setReturnArrays",
              "meta": {
                "added": [
                  "v24.0.0",
                  "v22.16.0"
              ...
              "signatures": [
                {
                  "params": [
                    {
                      "textRaw": "`enabled` {boolean} Enables or disables the return of query results as arrays.",
                      "name": "enabled",
                      "type": "boolean",
                      "desc": "Enables or disables the return of query results as arrays."
                    }
                  ]
                }
              ],
              "desc": "<p>When enabled, query results returned by the <code>all()</code>, <code>get()</code>, and <code>iterate()</code> methods will be returned as arrays instead\nof objects.</p>"
            },
            {
          ...
          "properties": [
            {
              "textRaw": "`expandedSQL` Type: {string} The source SQL expanded to include parameter values.",
              "type": "string",
              "name": "Type",
              "meta": {
                "added": [
            ...
            {
              "textRaw": "`sourceSQL` Type: {string} The source SQL used to create this prepared statement.",
              "type": "string",
              "name": "Type",
              "meta": {
                "added": [
          ...
          "methods": [
            {
              ...
              "signatures": [
                {
                  "return": {
                    "textRaw": "Returns: {Array} An array of objects representing the rows returned by the query.",
                    "name": "return",
                    "type": "Array",
                    "desc": "An array of objects representing the rows returned by the query."
                  },
                  "params": [
                    {
                      "textRaw": "`sqlTemplate` {Template Literal} A template literal containing the SQL query.",
                      "name": "sqlTemplate",
                      "type": "Template Literal",
                      "desc": "A template literal containing the SQL query."
                    },
                    {
                      "textRaw": "`...values` {any} Values to be interpolated into the template literal.",
                      "name": "...values",
                      "type": "any",
                      "desc": "Values to be interpolated into the template literal."
                    }
                  ]
                }
              ],
              "desc": "<p>Executes the given SQL query and returns all resulting rows as an array of objects.</p>"
            },
            {
              "textRaw": "`sqlTagStore.get(sqlTemplate[, ...values])`",
              "type": "method",
              "name": "get",
              "meta": {
                "added": [
              ...
              "signatures": [
                {
                  "return": {
                    "textRaw": "Returns: {Object | undefined} An object representing the first row returned by the query, or `undefined` if no rows are returned.",
                    "name": "return",
                    "type": "Object | undefined",
                    "desc": "An object representing the first row returned by the query, or `undefined` if no rows are returned."
                  },
                  "params": [
                    {
                      "textRaw": "`sqlTemplate` {Template Literal} A template literal containing the SQL query.",
              ...
              "desc": "<p>Executes the given SQL query and returns the first resulting row as an object.</p>"
            },
            {
              "textRaw": "`sqlTagStore.iterate(sqlTemplate[, ...values])`",
              "type": "method",
              "name": "iterate",
              "meta": {
                "added": [
              ...
              "signatures": [
                {
                  "return": {
                    "textRaw": "Returns: {Iterator} An iterator that yields objects representing the rows returned by the query.",
                    "name": "return",
                    "type": "Iterator",
                    "desc": "An iterator that yields objects representing the rows returned by the query."
                  },
                  "params": [
                    {
                      "textRaw": "`sqlTemplate` {Template Literal} A template literal containing the SQL query.",
              ...
              "desc": "<p>Executes the given SQL query and returns an iterator over the resulting rows.</p>"
            },
            {
              "textRaw": "`sqlTagStore.run(sqlTemplate[, ...values])`",
              "type": "method",
              "name": "run",
              "meta": {
                "added": [
              ...
              "signatures": [
                {
                  "return": {
                    "textRaw": "Returns: {Object} An object containing information about the execution, including `changes` and `lastInsertRowid`.",
                    "name": "return",
                    "type": "Object",
                    "desc": "An object containing information about the execution, including `changes` and `lastInsertRowid`."
                  },
                  "params": [
                    {
                      "textRaw": "`sqlTemplate` {Template Literal} A template literal containing the SQL query.",
              ...
              "desc": "<p>Executes the given SQL query, which is expected to not return any rows (e.g., INSERT, UPDATE, DELETE).</p>"
            },
            {
              "textRaw": "`sqlTagStore.size()`",
              "type": "method",
              "name": "size",
              "meta": {
                "added": [
              ...
              "signatures": [
                {
                  "return": {
                    "textRaw": "Returns: {integer} The number of prepared statements currently in the cache.",
                    "name": "return",
                    "type": "integer",
                    "desc": "The number of prepared statements currently in the cache."
                  },
                  "params": []
                }
              ],
              "desc": "<p>A read-only property that returns the number of prepared statements currently in the cache.</p>"
            },
            {
              "textRaw": "`sqlTagStore.reset()`",
              "type": "method",
              "name": "reset",
              "meta": {
                "added": [
              ...
              "desc": "<p>Resets the LRU cache, clearing all stored prepared statements.</p>"
            },
            {
              "textRaw": "`sqlTagStore.clear()`",
              "type": "method",
              "name": "clear",
              "meta": {
                "added": [
              ...
              "desc": "<p>An alias for <code>sqlTagStore.reset()</code>.</p>"
            }
          ],
          "properties": [
            {
              "textRaw": "`capacity` Returns: {integer} The maximum number of prepared statements the cache can hold.",
              "type": "integer",
              "name": "return",
              "meta": {
                "added": [
              ...
              "desc": "<p>A read-only property that returns the maximum number of prepared statements the cache can hold.</p>",
              "shortDesc": "The maximum number of prepared statements the cache can hold."
            },
            {
              "textRaw": "`db` {DatabaseSync} The `DatabaseSync` instance that created this `SQLTagStore`.",
              "type": "DatabaseSync",
              "name": "db",
              "meta": {
                "added": [
              ...
              "desc": "<p>A read-only property that returns the <code>DatabaseSync</code> object associated with this <code>SQLTagStore</code>.</p>",
              "shortDesc": "The `DatabaseSync` instance that created this `SQLTagStore`."
            }
          ],
          "modules": [
            {
      ...
      "methods": [
        {
          ...
          "signatures": [
            {
              "return": {
                "textRaw": "Returns: {Promise} A promise that fulfills with the total number of backed-up pages upon completion, or rejects if an error occurs.",
                "name": "return",
                "type": "Promise",
                "desc": "A promise that fulfills with the total number of backed-up pages upon completion, or rejects if an error occurs."
              },
              "params": [
                {
                ...
                },
                {
                  ...
                  "options": [
                    ...
                    {
                      "textRaw": "`progress` {Function} An optional callback function that will be called after each backup step. The argument passed to this callback is an {Object} with `remainingPages` and `totalPages` properties, describing the current progress of the backup operation.",
                      "name": "progress",
                      "type": "Function",
                      "desc": "An optional callback function that will be called after each backup step. The argument passed to this callback is an {Object} with `remainingPages` and `totalPages` properties, describing the current progress of the backup operation."
                    }
                  ]
                }
              ]
