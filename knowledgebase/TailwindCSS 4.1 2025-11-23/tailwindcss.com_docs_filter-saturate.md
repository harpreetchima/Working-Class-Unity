...
Use the`saturate-[<value>]`syntaxto set the saturation based on a completely custom value:
...
Prefix a`filter: saturate()` utilitywith a breakpoint variant like `md:` to only apply the utility at mediumscreen sizes and above:
